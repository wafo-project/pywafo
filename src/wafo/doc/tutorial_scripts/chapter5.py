...
YI = si.interp1d(xn[:, 0], xn[:, 1], kind='linear')(XI[:N])
YI = YI.reshape(n5min, N // n5min)  # Each column holds 5 minutes of interpolated data.
...
gm = ws.genextreme.fit2(Rgev, *gp.par.ravel().tolist(), method='ml')
gm.plotfitsummary()
plt.show(block=True)
...
sT = Y5gev.isf(1. / T)
ci = Y5gev.ci_quantile(sT, alpha=0.05, i=2)
sTlo, sTup = ci.T
...
# POT- method
gpd7 = ws.genpareto.fit2(Hs[Hs > 7], method='mps', floc=7)
