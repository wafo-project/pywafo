...
__all__ = ['kde_demo1', 'kde_demo2', 'kde_demo3', 'kde_demo4', 'kde_demo5', 'kreg_demo1']
...
def check_bkregression(hs=None, sizes=(50, 100, 300, 600), plot_score=True):
    """
    ...
    """
    # plt.ion()
    plotshape = (2, 2) if plot_score else (2, 1)
    options = dict(alpha=0.05, color='g')
    for k, n in enumerate(sizes):
        x, y, fun1 = _get_data(n, symmetric=False, loc1=1.8, scale1=0.5, scale2=0.6)
        figure_k, axes = plt.subplots(*plotshape, num=k)
        for method, axesi in zip(['beta', 'wilson'], axes):
            bkreg = BKRegression(x, y, a=0.05, b=0.05, method=method)
            prb_e = bkreg.prb_empirical(**options)
            if hs is None:
                fbest = bkreg.prb_search_best(prb_e, hsfun='hste', label='Transit_D', **options)
            else:
                fbest = bkreg.prb_smoothed(prb_e, hs, alpha=0.05, color='g')
            if plot_score:
                ax1 = axesi[1]
                fbest.score.plot(axis=ax1)
                axsize = ax1.axis()
                ax1.vlines(fbest.hs,axsize[2]+1,axsize[3])
                ax1.set(yscale='log')
                ax1.set_title('AICC')
                ...
                ax = axesi
            fbest.labels.title = 'N = {:d}, method = {}'.format(n, method)
            fbest.plot(axis=ax)
            xi = np.linspace(x.min(), x.max(), 600)
            yi = fun1(xi)

            ax.plot(xi, yi, 'r', label='True Probability')
            ax.legend(frameon=False, markerscale=4)
            ax.grid(True)

    if fig:
        fig.tile(list(range(len(sizes))))
    # plt.ioff()
    # plt.show('hold')
...
if __name__ == '__main__':
    ...
    # kde_demo5()
    # check_bkregression(hs=0.1, plot_score=False)
    # check_bkregression()
