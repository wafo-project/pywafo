...
log.info("Tc mean : {}".format(Tc.mean()))
log.info("Tc maxi : {}".format(Tc.max()))

t = np.linspace(0.01,8,200);
ftc = wk.TKDE(Tc, L2=0, inc=128)

plt.plot(t,ftc.eval_grid(t), t, ftc.eval_grid_fast(t),'-.', color='g')
wm.plot_histgrm(Tc,normed=True)
plt.title('Kernel Density Estimates')
plt.axis([0, 8, 0, 0.5])
set_windows_title('Kernel Density Estimates')
...
indS = S.argmax()
indH = H.argmax()
log.info("wave indices to plot S.argmax = {} H.argmax = {}".format(indS, indH))
ts.plot_sp_wave([indH, indS],'k.', color='r')
plt.ioff()
set_windows_title('Wave Height Steepness')

#  Does the highest wave contradict a transformed Gaussian model?
# ----------------------------------------------------------------

# TODO: Fix this. All below is Matlab format. Needs to convert to python
...
#plt.ioff()
#set_windows_title('Wave Parameters')
#
#fac = kde(Ac,{'L2',L2},np.linspace(0.01,3,200))
#plt.pdfplot(fac)
#wafostamp([],'(ER)')
