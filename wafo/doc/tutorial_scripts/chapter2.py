...
# switch interactive on in order to prevent blocking of the script after a plot
plt.ion()
...
lc.plot()
set_windows_title("Level crossing", log)
...
log.info("fm = {}, alpha = {}, ".format(fm, alfa))
...
plt.axis([0, 2, -2, 2])
set_windows_title("Wave  level", log)
...
S.plot()
plt.axis([0, 5, 0, 0.7])
set_windows_title("Periodogram: Raw spectrum", log)

# Calculate moments
#-------------------
mom, text = S.moment(nr=4)
log.info("sigma = {}, m0 = {}".format(sa, np.sqrt(mom[0])))
...
S1.plot('-.')
S2.plot()
set_windows_title("Spectral Density Compare L={} and L={}".format(Lmax0, Lmax1), log)
...
R2.plot('.')
Rest.plot()
set_windows_title("Auto covariance real vs estimate 1 (top) and 2 (bot)", log)
...
g.plot('r')
plt.grid('on')
set_windows_title("Transformation compared", log)
...
# "light" lower tail.
plt.figure()
ws.probplot(ts.data.ravel(), dist='norm', plot=plt)
set_windows_title("Prob plot", log)
...
spec.plot()
set_windows_title("Johnswap Spectral Density", log)
...
Sd.plot()
set_windows_title("Spreading function", log)
